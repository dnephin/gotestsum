...
func rerunFailed(ctx context.Context, opts *options, scanConfig testjson.ScanConfig) error {
	...
	rec := newFailureRecorder(nil)
	rec.failuresFromExecution(scanConfig.Execution)

	var lastErr error
	for count := 0; rec.failures() > 0 && count < opts.rerunFailsMaxAttempts; count++ {
...
func (r *failureRecorder) failuresFromExecution(exec *testjson.Execution) {
...
func (r *failureRecorder) failures() int {
	total := 0
...
func (r *failureRecorder) reset() {
	r.pkgFailures = make(map[string][]string)
}
