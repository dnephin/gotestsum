...
update-golden() {
    gotestsum -- . ./testjson ./internal/junitxml ./cmd/tool/{slowest,metrics} -test.update-golden
}

update-golden-all-go-versions() {
    update-golden
    GOLANG_VERSION=1.13-alpine ./do shell bash -c 'go test . -test.update-golden'
}
